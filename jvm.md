...
CMS收集器是一种以获取最短停顿时间为目标的收集器。它适用于注重用户体验的应用上。
...
对标记的对象进行清除回收，清除的过程中，可能仍然还有新的垃圾产生，这些垃圾就叫浮动垃圾。
...
- **volatile 关键字只能保证可见性，并不能保证原子性**。只适用于一个写，一个或者多个读的情况。
