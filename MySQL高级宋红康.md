...
自适应哈希索引：哈希查找速度会特别快。查询B+树相较于哈希还是慢一些的。InnoDB会监控表上索引页的查询，如果观察到建立哈希索引能够性能提升，则建立哈希索引。读取和写入速度可以提高2倍，默认开启自适应哈希索引。
...
B+树的数据删除使用填充因子控制树的变化。默认值是50%。叶子节点的容量节点数占用页节点空间小于50%时，缩容。
...
**==聚簇索引B+树找到一个给定主键值的具体行数据。B+树先目标数据所在的叶子节点（页），然后将该页读取到内存中，在从该页中查找，得到最后的数据。==**
...
- **再次验证了select，where 操作是不同时运行的，只有在这种特殊情况下，才能跳出select顺序执行的逻辑。（没太理解）**
...
在处理数据库数据时，构建缓冲池，总是先修改内存中的数据。然后再更新到数据库。缓冲池可以帮助我们消除CPU和磁盘之间的鸿沟。**由于事务的持久性，修改内存和修改磁盘有时间间隔，也不会每次操作都修改磁盘，操作时间慢，所以使用了redo日志**。redo日志记录事务处理的所有操作，并保存在磁盘中。内存每1s进行刷盘。若服务器宕机，则按redo日志记录操作进行持久化操作。
...
InnoDB可以通过修改innodb_flush_log_at_trx_commit参数可以更改刷盘策略。即如何将 redo log buffer 中的日志刷新到 redo log file 中
...
由于MVCC和事务，在事务执行过程中和提交后，短时间内不会清除undo log日志。数据库会通过**后台 Purge 线程**主动清理 “不再需要的`undo log`”，回收磁盘空间

1. **判断 “可清理” 的条件**

   1. Purge 线程会定期检查`undo log`，满足以下条件则标记为 “可清理”：

      - 对应的事务已提交（排除未提交事务的回滚需求）；

      - 数据库中所有**活跃事务的 Read View**（MVCC 的快照视图）都 “看不到” 该`undo log`对应的历史版本（即没有读操作需要该日志）。
        简单说：当`undo log`既不需要用于回滚，也不需要用于给读操作提供历史版本时，就会被清理。

2. 清理后的空间复用**
   清理`undo log`后，释放的空间不会立即 “归还” 给操作系统（除非手动触发表空间收缩），但会被**新的`undo log`复用**。

   - 例：若清理了 100MB 的`undo log`，后续新生成的`undo log`会直接使用这部分空闲空间，而非持续占用新的磁盘空间。

   - 只有当`undo log`的生成速度**远大于清理速度**时（如短时间内高频修改 + 存在长事务），才会出现磁盘占用短期增长；一旦长事务结束，Purge 线程会快速回收空间，磁盘占用会趋于稳定。
...
CheckPoint技术：选择一定的时机将缓存池中的脏页刷会到磁盘。有两种CheckPoint技术
...
在执行数据库修改操作时，宕机，通过redo log日志进行恢复。redo log日志大小有限制，数据库完全依赖刷盘后的redo log日志来恢复从数据库启动时的所有数据，效率太差，而且redo log日志也不能无限正常，需要循环使用。需要CheckPoint指针进行标记，在CheckPoint指针标记点后面的redo log日志进行恢复。**若是redo log日志被覆盖重写时，发现redo log日志还需要使用，此时通过CheckPoint技术进行刷盘**
...
- InnoDB的行锁
  - 记录锁：与表锁特性相同   共享锁，排他锁
  - 间隙锁：**只为了解决幻读问题，**禁止该行所在间隙插入数据。共享和独占锁作用相同。**多个事务可以对同一个间隙内加锁，间隙锁不互斥**
...
- Prepare阶段：事务写入redolog 日志并持久化，事务未最终提交
- Commit阶段：事务将操作记录写入binlog，并持久化，持久化成功后事务状态修改为Commit。

**因此两阶段提交是检测bin log日志是否写入成功。若没有成功则回滚，成功则不必回滚**
...
- char(n)
  - 有长度限制，0<=n<=255，n是字符个数
  - 若存储数据小于n个字符，会在右侧填充空格，读取时会自动移除
  - 因为长度固定，所以查找速度会快一些，但长度固定，如果存储长度不固定，则会有数据冗余
  - 超过char和varchar的n设置后，字符串会被截断。
- vachar(n)
  - 有长度限制，0<=n<=65532，n是字符个数
  - 长度可调整，超过char和varchar的n设置后，字符串会被截断。
- Text
  - 存变长大数据，速度慢，不存在空间浪费，不处理尾部空格，上限65535，会用额外空间存放数据长度，可以全部使用65535。
