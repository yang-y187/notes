...
- float是单精度浮点数（有效位数6-7位），double是双精度浮点数（有效位数 15-17位），1.1 是二进制中无限循环的小数。
- 两个值比较时，会将float隐式的转换为double 类型，但精度仍有部分差异
...
|              | Go (Golang)                                                  | Java                                                         |
| :----------- | :----------------------------------------------------------- | ------------------------------------------------------------ |
| **设计哲学** | **简单、高效**：关键字少，语法简洁，强调“一件事只有一种做法”。 | **全面、强大**：语言特性丰富，面向对象极致，强调工程的严谨性。 |
| **范式**     | 主要支持**命令式**和**并发**范式。有简单的结构体和方法，但不是传统的面向对象。 | **纯面向对象**（除了基本类型）。全面支持面向对象、函数式、泛型编程。 |
| **语法风格** | **极简主义**。省略不必要的符号（如分号、括号），强制统一的代码格式（`gofmt`）。 | **显式且规范**。语法更冗长，但意图非常清晰。                 |
| **并发模型** | **CSP (Communicating Sequential Processes)**：核心是 **Goroutine** (轻量级线程) 和 **Channel** (通道)。语言原生支持，非常简单高效。 | **基于线程**：使用 `Thread` 类和 `java.util.concurrent` 包。功能强大但更重量级，需要开发者精细管理。 |
| **类型系统** | **静态强类型**，但支持**类型推断**（`:=`）。早期缺乏泛型（1.18后已加入），接口是**鸭子类型**。 | **静态强类型**。拥有成熟且强大的**泛型**系统。接口是显式实现。 |
| **内存管理** | **自动垃圾回收 (GC)**。GC 设计目标之一是**低延迟**（STW时间短），尤其适合后端服务。 | **自动垃圾回收 (GC)**。拥有极其成熟且高度可调的GC器（如G1, ZGC），追求高吞吐量或低延迟。 |
| **运行时**   | 编译为**静态二进制文件**。无需任何外部依赖或虚拟机，直接运行于操作系统。 | 编译为**字节码**，运行在 **JVM (Java虚拟机)** 上。需要安装JRE。 |
| **性能**     | **启动速度快**，**内存占用低**。运行时性能与Java相当，在某些场景（如并发、网络服务）更优。 | **预热后性能极高**（得益于JIT即时编译）。但启动时间较慢，内存占用较高。 |
| **错误处理** | **显式错误检查**。通过多返回值返回错误，要求开发者立即处理（`if err != nil`）。 | **异常机制**。通过 `try-catch-finally` 进行结构化错误处理，允许错误冒泡。 |
| **生态系统** | **现代且聚焦**：在**云原生**（Docker, Kubernetes, Istio）、API、CLI工具领域是事实标准。库数量少于Java。 | **庞大而成熟**：拥有海量的库、框架（Spring, Hibernate）和企业级解决方案，覆盖所有领域。 |
| **学习曲线** | **非常平缓**。语法简单，概念少，新手很容易上手并写出高效的代码。 | **相对陡峭**。需要理解面向对象、异常、复杂的类库和JVM概念才能精通。 |

## go为什么快

- 使用协程，高并发高吞吐：协程**轻量、低成本**，1个协程只占用2kb，一个 Java 线程默认栈大小是 **1MB**（并且通常固定不变）。并且高效调度，M:N模型，M个任务交给N个执行器执行
- 直接编译为机器码：无需编译解释。
...
Unsafe类是仅为Java核心库内部使用的类，提供了绕开JVM安全机制、直接操作内存和执行底层硬件原子的操作

**总结与注意事项**

| 功能                     | 底层原理                          | 应用场景                                     |
| :----------------------- | :-------------------------------- | :------------------------------------------- |
| **对直接内存部分的操作** | 编译为 CPU 内存指令，操作堆外内存 | Netty, Kafka, 零拷贝                         |
| **CAS**                  | CPU 原子指令 (如 `CMPXCHG`)       | `AtomicInteger`, `AQS`, 无锁并发             |
| **内存屏障**             | CPU 内存屏障指令 (如 `MFENCE`)    | `volatile`, 无锁数据结构，保证可见性         |
| **对象操作**             | 直接操作 JVM 对象内存布局         | 序列化/反序列化框架                          |
| **线程调度**             | 与 JVM 线程调度器交互             | `LockSupport`, Java 锁机制、Park和UnPark方法 |

**重要警告**：
虽然 `Unsafe` 功能强大，但正如其名，它是**不安全**的。错误使用会导致：

- **JVM 崩溃**：直接操作内存时地址计算错误。
- **内存泄漏**：分配堆外内存后忘记释放。
- **不可移植性**：内存布局、字节顺序等可能因 JVM 实现和硬件平台而异。
- **破坏语言安全性**：绕过访问控制，破坏封装性。

正因如此，在 Java 9 及以后的版本中，官方正在用更安全、更可控的 **`VarHandle`** API 来逐步替代 `Unsafe` 的许多功能。`VarHandle` 提供了类似的内存操作和原子操作，但具备了更好的访问控制和类型安全。对于新项目，应优先考虑使用 `VarHandle` 而不是直接使用 `Unsafe`。
...
7. ```java
   ...
   System.out.println(new TestB()); // C F B E D
   ```
