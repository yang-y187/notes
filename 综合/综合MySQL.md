...
**spring的默认隔离级别是：default 取数据库的隔离级别**
...
2. null值列表（逆序存储）
   ...
   - 为什么要存在null值列表？
     ...
     2. 根据列的值，填充 0或1 。 1表示为空「该列表实际是0100011这种二进制格式，逆序表示表每个字段列是否为null
        - **新增字段时的操作**：新增字段时，不会修改已有数据的null值列表
          - 当你执行 `ALTER TABLE table_name ADD COLUMN new_column INT DEFAULT NULL;` 时，数据库引擎主要做两件事：
            a. **更新元数据**：在系统表（如 `information_schema`）中记录这张表新增了一个字段，其默认值为 `NULL`。这是一个非常快的操作。
            - **修改NULL位图结构**：在每行记录的NULL位图中**新增一个位**，用来表示这个新字段是否为 `NULL`。对于已有数据，数据库**不会立即去修改每一行**的物理数据页。
          - **当发生读操作时**：任何查询在读取已有数据行时，数据库引擎会查看新的、更宽的NULL位图。对于这个新字段，由于没有存储实际值，并且位图中对应的位未被初始化（或会被智能地解释），引擎会直接返回 `NULL` 给用户。
          - **当发生写操作时**：当某一行被更新（`UPDATE`）时，数据库会用新的表结构（包含新字段的NULL位图）来重写这一行。此时，这个新字段的NULL位会被正式设置，但**仍然不会占用存储实际值的空间**，因为它的值是 `NULL`。
...
- 找到主键值比本条记录的主键值最近的组。该组的n_owned 值+1
