...
- discuss_post：id，usedId，标题，内容，评论数量，创建时间，热点评分（根据此来进行热点排序），

  - status（0：正常; 1-精华; 2-拉黑，帖子并未真正删除，而是改为不可见状态。）
  - type：0：普通帖子，1，指定帖子

- user：用户名，密码（加密加盐后的密码），salt（盐，与密码进行拼接，再进行加密），邮箱，头像网址

  - status：0-未激活; 1-已激活;
  - type：0-普通用户; 1-超级管理员; 2-版主;
  - 激活码：发送邮件激活时，网址：http://localhost:8080/userId/激活码（加激活码的目的是：避免恶意注册）
  - 发送邮件（指定邮件内容，收信人）

- comment: usedId，entity_type :实体类型：1，帖子评论，2，评论的评论。entity_id：被评论的id。status：状态：2为拉黑，create_time创建时间

- message：status：1，已读，2，未读
...
like:entity:1:294（postId）：set集合保存当前帖子点赞的用户Id

user:11(usedId)：登录的用户详细信息。
...
- **异常统一处理**
  ...
  - 使用@ExceptionHandler创建一个新的类，来处理异常，在每个方法处理特定异常。便于后期的拓展和维护
...
值得一提的是，帖子，平路，回复都需要查看点赞的状态，个数。所以需要在原来的controller中响应获取
