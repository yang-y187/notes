...
**尽量先使用组合或者聚合等关联关系来实现，其次才考虑使用继承关系来实现。**【桥接模式就是一个具体体现（图片可能是矩形，也可能是圆形，颜色可能是黑色，也可能是白色。如果是以形状和颜色作为一个抽象类或接口，进行继承实现，那么新增一种粉色三角形则是代码爆炸式增加，因此，继承关系限制了灵活性和复用性，聚合关系应该优先使用）】
...
java中，万物皆对象，对象创建需要new对象，耦合严重，如果更改对象，需要将所有new对象的位置都改。违背了软件设计的开闭原则。因此，我们选择工厂来生产对象，使用对象，只从工厂获取，如果更换对象，在工厂内更换，达到对象解耦的目的。工厂模式的最大优点：**解耦**
...
简单工厂不是设计模式，更多的像是一种编程习惯，通过逻辑判断 if-else或者switch选择合适的类。开发过程中，很多将工厂类里的创建对象功能设为静态的static，即静态工厂模式，此模式也是开发习惯，而非设计模式
...
简单工厂模式在新增产品对象后，仍需要更改工厂类对象，违背开闭原则。该工厂方法模式可以完美解决。工厂方法模式将对象的创建抽离成一个工厂，通过工厂创建一种对象。需要哪种对象时，创建哪种工厂即可。工厂和对象都有抽象类。
...
- 浅克隆
  ...
  - ```java
    实现cloneable接口，然后返回克隆对象
    ```

- 深克隆

  1. ```java
     JSON.parseObject(JSON.toJSONString(obj),Object.Class)  // fastjson  json化
     ```

  2. 手动修改 clone方法，将地址引用也赋值

  3. 据说有 工具类的方法deepclone，但没找到
...
- 分离了部件的构造和装配，可以构建出复杂的对象。 **该模式适用于某个对象的创建过程十分复杂，但是建造顺序稳定，对象内部创建面临复杂变化**
- 完成了构建对象和表示解耦，不同的构造器，相同的装配，可以是不同的对象；相同的构造器，不同的装配顺序，也可以构造不同的对象。
- 构造者模式，用户提供材料即可，不需要知道具体的构造细节

建造者模式：定义一个抽象建造者，指定哪些操作是必须的。具体的对象实现抽象类，重新构建方法。指挥者，聚合了抽象建造者，指定所有的创建步骤，创建出对象。使用时，则创建出指挥者和具体产品对象，将具体产品对象传入指挥者类
...
### 模式拓展
 ...
 建造者模式：我们目前的BPaas就是建造者模式，不允许直接创建内部类对象，而是通过传参builder，来创建我们想要的对象。若参数过多，则直接创建该对象，代码可读性很差，因为构造器包含所有参数，创建时传入过多参数，可读性太差，而且很容易引入错误。

 所有的请求流程都是，组装，校验，落库。所以这也可以抽离出一个指挥者。而每个请求的Service处理不一样，那么流程每个环节都可以实现接口，进行拓展。
...
**理解：**装饰者类：是将主食（被操作类）为主类。具体装饰类：各种配菜（装饰类，其内部既聚合了被操作类，又包含对该被操作类处理方式）。装饰者类将每个要装饰的功能抽离成一个类，并用该类包装所要装饰的对象。即方便拓展（继承创建新类），又内聚，外部松散（灵活），使用时，按需创建该类即可。将类的职责和装饰功能区分开。
...
- 体现了依赖倒装原则（高层依赖底层的抽象接口，而非琐碎详细的接口）和迪米特原则（最小依赖原则）
...
- 对分层结构系统构建时（MVC架构），通过外观模式，调用统一的接口，而不是子系统的具体实现，降低了耦合度
- 当一个复杂系统的子系统很多时，外观模式可以为系统设计一个简单的接口供外界访问。可理解为维护一个遗留的大型系统时，系统可能难以维护和拓展，则可以通过增加外观提供简单接口，通过与外观对象的交互，屏蔽了遗留代码的复杂
...
职责链模式，为避免请求发送者与多个请求处理者耦合在一起，将所有请求的处理者通过前一个对象记住后一个对象的引用，进行连成一个链路，将请求按链路传递。（更多的理解为走流程，需要走到各个部门处理，注意该处理是请求链路的每个节点，知道有对象处理它。手动的构建一个单向）
...
对于有状态的对象，把复杂的逻辑判断提取到不同的状态对象中，允许状态对象在其内部状态发生改变时，改变其行为。一种避免长方法的方式。

MartinFowler在《重构》里有写：长方法是有点坏味道。
...
- 抽象主题（抽象被观察者）：抽象主题角色把所有观察者保存在一个集合中，抽象主题提供一个接口，可以增加删除观察者对象。1对多的那个1
- 具体主题（具体被观察者），该角色将有关状态保存在具体观察者对象，在具体主题状态发生变化时，给所有注册的观察者发送通知
- 抽象观察者：观察者抽象类，有个更新接口，在主题更改通知时更新自己。1对多的那个n
...
- 对象存在一对多的关系，一个对象的状态发生改变影响其他对象，而且它不知道具体有多少对象有待改变时
- 当一个抽象模型有两个方面，其中一个方面依赖于另一个方面 ，使用观察者模式可以将两者封装在独立的对象中使它们各自独立地改变和复用。
...
（大多时候不需要访问者模式，一旦需要访问者模式，那么则 是真的需要访问者模式了）
...
![image-20230820162210709](%E8%AE%BE%E8%AE%A1%E6%A8%A1%E5%BC%8F.assets/image-20230820162210709.png)
...
- **优点**
  - 容易更改和拓展（每个文法规则都是一个类，容易拓展和修改）
  - 实现文法容易（定义抽象语法树中各个节点的类实现大体相似，易于编写）
  - 增加新的解释表达式较为方便（新增一个终结符表达式或非终结符表达式）
- **缺点**
  - 若包含很多规则的文法，难以维护（若文法非常负载时，建议使用语法分析程序或编译器生成器）
  - 执行效率低（解释器中使用了大量的循环和递归调用）

### 6.11.5 使用场景

- 当语言的文法较为简单，且对时间要求不高时
- 问题重复 出现，可以采用一种简单语言进行表达
- 当一个语言需要执行，并且语言中的句子可以作为文法
