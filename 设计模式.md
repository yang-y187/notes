...
- 装饰者模式比继承有更加灵活的扩展功能，使用方便，通过组合不同的装饰者对象获取具有不同行为状态的多样化结果。装饰者模式具有比继承更具良好的拓展性，符合开闭原则，继承是静态的附加责任，装饰者则是动态的附加责任
- 装饰类和被装饰类可以独立拓展，而没有相互耦合，装饰者模式是继承的一个替代模式，装饰模式可以动态的拓展一个实现类的功能。
...
- 当不能采用继承的方式对系统进行拖充或者采用继承不利于系统拓展和维护时
  不能采用继承的情况分为
  - 第一类是系统中存在大量独立的拓展，为支持每一种组合将产生大量的子类，是的子类数据呈爆炸性增长
  - 第二类是因为类的定义不能继承（比如 final）
- 在不影响其他对象的情况下，以动态，透明的方式给单个对象增加功能
- 当对象的功能要求可以动态增加时，也可以动态地撤销时
...
- 不同点:
  ...
  - 获取目标对象构建的地方不同
    装饰者模式是由外界传递进来，可以通过构造器传递，静态代理是由代理类内部构建，以此隐藏目标对象
...
- 拓展抽象化角色：抽象化角色的子类，实现父类中的业务方法，并通过组合关系调用实现化角色中的业务方法
