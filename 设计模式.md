...
与static没有任何关系，而是将代码写死，以车站卖车票为例，可以有多个代理点来卖车票。多个代理点则是代理对象，车站是目标对象。
...
:warning:ProxyFactory不是代理模式下的代理类，代理类是程序运行过程中在内存动态生成的类。也就是说该类没有暴露出来，只保存在内存中。内部结构如下

- 代理类（$Proxy0）实现了与真实类相同的接口
...
CGLIb是一个功能强大的代码生成包，在被代理类没有实现接口时，可以通过CGLIB完成代理。
