...
- #### 为什么jdk代理只能基于接口代理
  ...
  - 生成的代理对象会继承Proxy这个类（不是被代理类，是一个指定类），java是单继承关系。若是通过继承创建代理对象，则无法再继承Proxy对象，也无法创建该代理对象，因此只能通过接口代理。（说白了Proxy来创建被代理对象，调用其方法）
    - 通过继承该Proxy类实现所有的代理方法，实际是通过invoke调用
  - 静态代理有两种方式，也分别是cglib和jdk代理的实现方式
    - 继承需要代理的类，然后在代理类中进行一些操作，然后调用父类的方法 cglib
    - 使用接口的方法，代理类和被代理类实现相同的接口，然后代理类内部聚合了被代理类对象，执行方法时，执行完相应的逻辑后，调用被代理类的相应方法（这也表明如果代理对象的方法不是接口定义的方法，则无法代理） jdk动态代理
...
<img src="Spring.assets/v2-2644b426ce5de72ac3166297eff08023_720w.jpg" alt="img" style="zoom:150%;" />
